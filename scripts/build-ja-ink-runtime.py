...
OUT=ROOT/'output/posters/japanese'
...
scenes=[dict(id='ikeda',code='2720400',hazard='earthquake',place='大阪府池田市',label='地震发生',title='日常看天气，震时先护身。',sub='从所在地天气，到地震信息与一屏应对。',context='模拟前提：池田市震度 5弱、マグニチュード M6.2；所有数值均为演练设定。',guide='降低重心，保护头部',note='先保护自己；摇晃停止后，再查看信息与避难地点。',daily='大阪府池田市 · 晴 · 日间最高 28℃',high=28),dict(id='wakayama',code='3020100',hazard='volcano',place='和歌山县',label='架空の火山噴火',title='平时有所准备，灾时知道行动。',sub='以虚构火山想定，展示火山警戒信息与应对指南。',context='地理说明：和歌山及周边无活火山。本图仅为虚构演练，不代表当地风险评估。',guide='保护头部，遵循现场規制',note='查看火山警戒与規制信息；遵循当地政府及现场人员指示。',daily='和歌山县和歌山市 · 晴 · 日间最高 29℃',high=29),dict(id='iwate',code='0320200',hazard='tsunami',place='岩手县沿岸',label='地震 + 海啸警报',title='平日了解沿岸，警报时立即避难。',sub='确认所在沿岸，查看海啸警报与避难行动。',context='模拟前提：宫古市震度 5弱；岩手县沿岸发布津波警報，予想高さ 3m。均为訓練データ。',guide='不要等待警报或倒计时',note='沿岸感到强烈或持续摇晃：先防护摇晃，随后立即向高处避难。',daily='岩手县宫古市 · 晴 · 日间最高 24℃',high=24)]
for s in scenes:
 c=city(s['code']); coast='岩手県' if s['id']=='iwate' else ''
 common=dict(id='mamori-demo-'+s['id'],cityCode=c['code'],city=c['office']+' '+c['name'],publisher='MAMORI 訓練データ',status='訓練',infoType='発表',scope='municipality',activeState='observed')
 if s['id']=='ikeda':
  e=dict(common,hazard='earthquake',name='地震情報',intensity='5弱',metrics=[dict(label='市内の震度',value='5弱'),dict(label='マグニチュード',value='M6.2'),dict(label='震源',value='大阪府北部')])
 elif s['id']=='wakayama':
  e=dict(common,hazard='volcano',name='噴火警報',activeState='reported',metrics=[dict(label='想定',value='架空の火山噴火'),dict(label='警戒レベル',value='3 / 訓練'),dict(label='規制',value='入山規制')])
 else:
  e=dict(common,hazard='tsunami',name='津波警報',scope='confirmed-coast',activeState='reported',coast=coast,timingNote='到達時刻は未設定・直ちに避難',metrics=[dict(label='対象沿岸',value='岩手県'),dict(label='予想高さ',value='3m / 訓練')])
 events=[e]
 if s['id']=='iwate':events.append(dict(common,id='iwate-earthquake-demo',hazard='earthquake',name='地震情報',intensity='5弱',metrics=[dict(label='市内の震度',value='5弱'),dict(label='マグニチュード',value='M7.5'),dict(label='震源',value='三陸沖')]))
...
const memory={'mamori-confirmed-city':fixture.city.code,'mamori-confirmed-coast':fixture.coast,'mamori-language':'ja'};
const storage={getItem:k=>memory[k]||null,setItem:(k,v)=>{memory[k]=v},removeItem:k=>{delete memory[k]}};
export const store=()=>storage;
function sample(){const now=Date.now();const d=Math.floor((now+32400000)/86400000)*86400000-32400000;return {city:fixture.city,checkedAt:now,dataState:'fetched',warnings:{state:'fetched',alerts:[]},forecast:{issuedAt:now,stale:false,area:'訓練データ',days:[{dateTime:now,condition:'晴',high:fixture.high,station:'訓練',rainSlots:[{startsAt:d+Math.floor((now-d)/21600000)*21600000,probability:10}]}]},events:{coast:fixture.coast,items:fixture.events.map(e=>({...e,issuedAt:now})),eew:'not-connected'}};}
...
""".replace('FIXTURE',json.dumps(fixture,ensure_ascii=False))
 ...
 for f in cfg['initialState']['files']:
  ...
  if f['path']=='modules/alerts.js':
   ...
   src=src.replace("stateLabel:e.activeState", "stateLabel:e.status==='訓練'?(lang==='zh'?'模拟演练 · 非官方发布':'訓練データ・公式発表ではありません'):e.activeState")
