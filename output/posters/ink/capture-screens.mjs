...
const t=await taskSpace(8);const p=t.page('p1');
const scenes=JSON.parse(await fs.readFile(root+'/scenes.json','utf8'));
for(const s of scenes){
 for(const kind of ['daily','warning','alert','guide']){
  const url=`http://127.0.0.1:8790/runtime/${s.id}${['daily','warning'].includes(kind)?'-'+kind:''}.html?lang=zh&cityCode=${s.code}&page=${['daily','warning'].includes(kind)?'home':kind}&hazard=${s.hazard}`;
